...
Public Go SDK for the tln language.

The `tln` CLI is the canonical embedding of the compile + execute pipeline. This package exposes that same pipeline to other Go programs so they can run tln source against a host-supplied MCP caller without depending on `internal/` packages.
...
func run(ctx context.Context, src string) error {
    result, err := tln.RunWorkflow(ctx, src,
        tln.WithMCP(&myCaller{}),
        tln.WithFilename("my_workflow.tln"),
    )
    if err != nil {
        return err
    }
...
```go
result, err := tln.Run(ctx, src,
    tln.WithDatalevinURL("http://localhost:8898"),
    tln.WithMCP(&myCaller{}),
)
```
...
```go
result, err := tln.Run(ctx, src, tln.WithFactStore(myStore))
```
...
| `WithFilename(name)` | Labels diagnostics with this filename. Defaults to `"<workflow>"` for `RunWorkflow`, `"<tln>"` for `Run`, `"<seed>"` for `Seed`. |
...
A rule's `do` clauses come back on the result as data — tln decides which
...
- Test-runner / REPL / trace / explain helpers — those live in the `tln` CLI.
