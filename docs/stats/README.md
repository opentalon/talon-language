...
Two complementary additions to Talon for working with statistical claims
and analyst tooling:
...
- **[CSV export](./csv-export.md)** — `talon explain --csv` writes Decisions
...
- **Mann-Kendall trend test** — non-parametric monotonic trend detection.
  Needs per-entity time-series storage that Talon's testrunner doesn't
...
- **R client SDK** — would let R users embed Talon rules in their pipelines.
