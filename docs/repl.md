...
`tln repl` is an interactive read-eval-print loop. Assert facts, define
...
an LLM extracts facts from claim invoices, tln makes the deterministic
...
```
$ tln repl
tln dev — type :help for commands, :quit to exit
...
`:eval` tells you the answer. `:trace` tells you how tln got there.

```
tln> :trace "Auto-approve in-network routine"
  "Auto-approve in-network routine": 1 detection(s) — records [902]
    trace:
...
tln> :count attr "provider_status" == "in_network"
  2

tln> :count attr "amount_chf" > 1000
  1
```
...
```
tln> record 999 type "claim"
  OK: record 999
tln> attr 999 "provider_status" "in_network"
  OK: attr 999
tln> attr 999 "service_category" "outpatient"
  OK: attr 999
tln> attr 999 "amount_chf" 320
  OK: attr 999
tln> attr 999 "per_visit_cap" 500
  OK: attr 999
tln> :eval "Auto-approve in-network routine"
...
```
tln> record 501 type "item" status "active"
  OK: record 501
tln> attr 501 "name" "VW Transporter"
  OK: attr 501
tln> record 502 type "item" status "active"
  OK: record 502
tln> attr 502 "name" "Ford Transit"
  OK: attr 502
tln> :eval "Active items"
...
- `:connect`, `:mcp` — connected-mode MCP tool calls (needs the MCP layer
  threaded through the session; tracked under [#17](https://github.com/opentalon/tln-language/issues/17)).
